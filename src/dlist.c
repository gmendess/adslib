...
void ads_dlist_pop_back(ads_dlist_t* dlist, void** ret_data) {
  ...
  ads_dlist_node_t* old_tail = ads_dlist_get_tail(dlist);
  if(ret_data)
    *ret_data = old_tail->data;
  else
    dlist->destroy(old_tail->data);

  dlist->tail = old_tail->prev;
